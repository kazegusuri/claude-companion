...
func TestHookEvent_ParseHookContent(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			content:           "\u001b[1mStop\u001b[22m [/test/bin/hook.sh] completed successfully",
			wantHookEventType: "Stop",
			wantHookCommand:   "/test/bin/hook.sh",
			wantHookStatus:    "finished",
			wantHookName:      "hook.sh",
			wantErr:           false,
		},
		{
			name:              "SessionStart:resume event with ANSI codes",
			content:           "\u001b[1mSessionStart:resume\u001b[22m [/test/bin/hook.sh] completed successfully",
